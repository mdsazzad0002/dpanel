...
Usage:
  ...
  filemanager user create <username> [--home <path>] [--shell <shell>] [--site-directory <dir>]
  filemanager user ensure <username> [--home <path>] [--shell <shell>] [--site-directory <dir>]
...
filemanager_prepare_site_directory() {
  local username="$1"
  ...
  site_directory="${site_directory:-public_html}"
  site_directory="${site_directory#/}"
  site_directory="${site_directory%/}"
  [[ -n "$site_directory" ]] || site_directory="public_html"

  site_path="${home%/}/${site_directory}"
  filemanager_validate_target "$site_path"
  group="$(filemanager_user_group "$username")"

  install -d -m 0750 -o "$username" -g "$group" -- "$site_path"
  if [[ -d "$site_path" ]]; then
    chown -R "$username:$group" -- "$site_path"
    chmod 0750 -- "$site_path"
  fi
}
...
filemanager_user_create() {
  ...
  local site_directory="public_html"
  ...
  filemanager_prepare_site_directory "$username" "$site_directory" "$home_dir"
