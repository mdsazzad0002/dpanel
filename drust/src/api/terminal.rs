...
fn execute(request: &Request) -> Result<(String, i32, String), String> {
    ...
    let root = Path::new(&request.project_root)
        .canonicalize()
        .map_err(|_| "Project root is unavailable.".to_string())?;
    ...
    for part in relative_parts
        .iter()
        .take(relative_parts.len().saturating_sub(1))
    {
        sandbox_parent.push(part.as_os_str());
        command.args(["--dir", sandbox_parent.to_string_lossy().as_ref()]);
    }
    let tracked_command = format!(
        "{}; __dpanel_status=$?; printf '\\n__DPANEL_CWD__%s\\n' \"$PWD\"; exit $__dpanel_status",
        request.command
    );
    command.args([
        "--bind",
    ...
    let output = command
        .current_dir(&root)
        .output()
        .map_err(|error| format!("Unable to start sandbox: {error}"))?;
    let stdout = String::from_utf8_lossy(&output.stdout);
    let marker = "__DPANEL_CWD__";
    let next_directory = stdout
        .lines()
        .rev()
        .find_map(|line| line.strip_prefix(marker))
        .unwrap_or(&working_directory_value)
        .to_string();
...
fn validate(request: &Request) -> Result<(), String> {
    let owner_ok = !request.site_owner.is_empty()
        && request.site_owner.chars().enumerate().all(|(i, c)| {
    ...
    let canonical = root
        .canonicalize()
        .map_err(|_| "Invalid project root.".to_string())?;
    let owner_home = Path::new("/home")
        .join(&request.site_owner)
        .canonicalize()
        .map_err(|_| "Invalid website owner home.".to_string())?;
    let inside_owner_home = canonical == owner_home || canonical.starts_with(&owner_home);
    if !inside_owner_home
        || !canonical.is_dir()
        || root
            .components()
    ...
    if command.is_empty()
        || command.chars().count() > 2000
        || command.contains('\0')
        || command.contains('\n')
        || command.contains('\r')
    {
        return Err("Enter one command of at most 2000 characters.".into());
    }
    ...
    if blocked.iter().any(|item| lower.contains(item)) {
        return Err("This command is blocked in the website terminal.".into());
    }
    Ok(())
}
