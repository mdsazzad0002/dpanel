...
fn execute(req: &Request) -> Result<serde_json::Value, String> {
    ...
    let expected = if matches!(req.framework.as_str(), "laravel" | "codeigniter4") { ".env" } else if req.framework == "wordpress" { "wp-config.php" } else if req.framework == "codeigniter3" { "config.php" } else { return Err("unsupported framework".into()); };
...
fn update_codeigniter_env(mut text: String, prefix: &str, host: &str, port: u16, db: u8) -> String {
    for (key, value) in [
        ...
        if let Some(old) = text.lines().find(|candidate| {
            candidate.trim_start_matches('#').trim_start().starts_with(&format!("{key} ="))
        }).map(str::to_owned) {
