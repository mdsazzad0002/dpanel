...
fn apply(request: &Request) -> Result<PathBuf, String> {
    ...
    let original = if path.is_file() {
        ...
        fs::read_to_string(&template).map_err(|error| {
            format!("CodeIgniter .env and env template are unavailable: {error}")
        })?
    } else {
...
fn validate(request: &Request) -> Result<(), String> {
    ...
    if !path.is_file()
        && !(request.framework == "codeigniter4" && path.with_file_name("env").is_file())
    {
        return Err("Project configuration file does not exist.".into());
    }
    for (value, label) in [
    ...
    for (key, value) in [
        ...
        let existing = text
            ...
            .find(|line| {
                ...
                normalized.starts_with(key) && normalized[key.len()..].trim_start().starts_with('=')
            })
            .map(str::to_owned);
...
mod tests {
    ...
    fn request(framework: &str) -> Request {
        Request {
            ...
            config_path: "/home/example/config".into(),
            database_name: "app_db".into(),
            database_user: "app_user".into(),
    ...
    fn updates_codeigniter4_commented_env_values() {
        let output = update_codeigniter4(
    ...
    fn updates_codeigniter3_default_connection() {
        let input = [
            "hostname", "username", "password", "database", "dbdriver", "port",
        ]
