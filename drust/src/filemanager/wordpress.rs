...
pub(crate) async fn handle(
    ...
    match tokio::task::spawn_blocking(move || prepare_target(&username, &path)).await {
        ...
        Err(error) => return ApiResponse::error(&format!("WordPress preflight worker failed: {error}")).into_response(),
    }
    ...
    {
        Ok(client) => client,
        Err(error) => return ApiResponse::error(&format!("WordPress download client failed: {error}")).into_response(),
    };
        ...
        .await
    {
        ...
        Err(error) => return ApiResponse::error(&format!("WordPress download failed: {error}")).into_response(),
    };
    if !response.status().is_success() {
        return ApiResponse::error(&format!("WordPress download returned {}", response.status())).into_response();
    }
    let package = match response.bytes().await {
        Ok(bytes) if bytes.len() <= MAX_PACKAGE_BYTES => bytes.to_vec(),
        Ok(_) => return ApiResponse::error("WordPress package exceeds the download limit").into_response(),
        Err(error) => return ApiResponse::error(&format!("WordPress download failed: {error}")).into_response(),
    };
    ...
    match result {
        ...
        Err(error) => ApiResponse::error(&format!("WordPress install worker failed: {error}")).into_response(),
    }
}
...
fn install_package(username: &str, target: &str, package: &[u8]) -> Result<(), String> {
    ...
    for index in 0..archive.len() {
        let mut entry = archive.by_index(index).map_err(|error| format!("Failed to read WordPress entry: {error}"))?;
        let path = safe_entry_path(&entry, index)?;
        let relative = path.strip_prefix("wordpress").map_err(|_| "WordPress package layout is invalid".to_string())?;
        ...
        let parent = ensure_directory_tree(&target, relative.parent().unwrap_or_else(|| Path::new("")))?;
        ...
        let mut output = fs::File::create(&temporary).map_err(|error| format!("Failed to create WordPress file: {error}"))?;
        io::copy(&mut entry, &mut output).map_err(|error| format!("Failed to extract WordPress file: {error}"))?;
        output.flush().map_err(|error| format!("Failed to flush WordPress file: {error}"))?;
        fs::rename(&temporary, &destination).map_err(|error| format!("Failed to install WordPress file: {error}"))?;
    }
    fix_extracted_tree(username, &group, &target)
}

fn ensure_installable_target(target: &Path) -> Result<(), String> {
    for entry in fs::read_dir(target).map_err(|error| format!("Cannot inspect document root: {error}"))? {
        let entry = entry.map_err(|error| format!("Cannot inspect document root entry: {error}"))?;
        let name = entry.file_name();
        let name = name.to_string_lossy();
        let allowed = match name.as_ref() {
            "index.php" => fs::read_to_string(entry.path()).map(|body| body.contains("@serverpanel-starter")).unwrap_or(false),
            "index.html" => fs::read_to_string(entry.path()).map(|body| body.contains("Starter page generated by ServerPanel")).unwrap_or(false),
            "extra" => managed_extra_only(&entry.path()),
            _ => false,
        };
        if !allowed {
            return Err(format!("WordPress install requires an empty document root; found: {name}"));
        }
    }
    Ok(())
}
...
fn managed_extra_only(path: &Path) -> bool {
    fs::read_dir(path).map(|mut entries| entries.all(|entry| entry.ok().is_some_and(|item| item.file_name() == "first-site-note.txt"))).unwrap_or(false)
}
...
fn remove_managed_demo(target: &Path) -> Result<(), String> {
    for name in ["index.php", "index.html", "extra"] {
        ...
        if path.is_dir() {
            fs::remove_dir_all(&path).map_err(|error| format!("Cannot remove managed demo folder: {error}"))?;
        } else if path.exists() {
            fs::remove_file(&path).map_err(|error| format!("Cannot remove managed demo file: {error}"))?;
        }
    }
    Ok(())
}
