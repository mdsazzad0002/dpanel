...
<template>
    <div class="space-y-6">
        <button type="button" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 transition-colors hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200" @click="router.visit(route('migrations.index', { token: panelToken }))">
            <i class="bi bi-arrow-left"></i> All migration services
        </button>
        ...
        <section class="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
            <div class="flex items-center gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-700">
                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-indigo-100 text-indigo-600 dark:bg-indigo-900/40 dark:text-indigo-300">
                    <i class="bi bi-terminal text-lg"></i>
                </span>
                <div class="min-w-0">
                    <h3 class="font-semibold text-slate-900 dark:text-white">Connect via SSH</h3>
                    <p class="truncate text-xs text-slate-500 dark:text-slate-400">CyberPanel, HestiaCP, aaPanel and Plesk are auto-detected. Credentials are used for the current request and stored only when you enable Save SSH access.</p>
                </div>
            </div>

            <div class="space-y-5 p-5">
                <div v-if="savedConnections.length" class="flex flex-col gap-2 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/70 sm:flex-row sm:items-center">
                    <span class="hidden shrink-0 items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-slate-400 sm:flex"><i class="bi bi-bookmark-star"></i></span>
                    ...
                    <button type="button" :disabled="!selectedConnectionId" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:opacity-40" @click="loadSavedConnection">Use connection</button>
                    <button type="button" :disabled="!selectedConnectionId" class="rounded-md border border-red-300 px-4 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:opacity-40 dark:border-red-800 dark:hover:bg-red-900/20" @click="deleteSavedConnection">Delete</button>
                </div>

                <form class="grid gap-4 md:grid-cols-2" @submit.prevent="inspect">
                    ...
                    <template v-else>
                        <label class="text-sm font-medium dark:text-slate-300 md:col-span-2">Private key<textarea v-model="ssh.private_key" required rows="7" placeholder="-----BEGIN OPENSSH PRIVATE KEY-----" class="mt-1 block w-full rounded-lg border-slate-300 font-mono text-xs dark:border-slate-700 dark:bg-slate-800"></textarea></label>
                        <label class="text-sm font-medium dark:text-slate-300 md:col-span-2">Key passphrase <span class="font-normal text-slate-400">(optional)</span><input v-model="ssh.key_passphrase" type="password" autocomplete="off" class="mt-1 block w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800" /></label>
                    </template>

                    <div class="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3.5 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-500/10 dark:text-amber-300 md:col-span-2">
                        <i class="bi bi-shield-exclamation mt-0.5 shrink-0 text-base"></i>
                    ...
                    <div class="md:col-span-2">
                        <button :disabled="inspecting" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:opacity-50">
                            <i class="bi bi-search"></i>{{ inspecting ? 'Connecting…' : 'Connect & discover' }}
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div v-if="message" class="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-800 dark:bg-emerald-500/10 dark:text-emerald-300">
            <i class="bi bi-check-circle-fill shrink-0"></i><span>{{ message }}</span>
        </div>
        <div v-if="error" class="flex items-center gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-800 dark:bg-red-500/10 dark:text-red-300">
            <i class="bi bi-exclamation-triangle-fill shrink-0"></i><span>{{ error }}</span>
        </div>

        <!-- Select migration source -->
        <section v-if="inventory" class="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
            <div class="flex flex-wrap items-center gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-700">
                <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-600 dark:bg-emerald-900/40 dark:text-emerald-300">
                    <i class="bi bi-hdd-network text-lg"></i>
                </span>
                <div class="min-w-0 flex-1">
                    <h3 class="font-semibold text-slate-900 dark:text-white">Select one migration source</h3>
                    <p class="truncate text-xs text-slate-500 dark:text-slate-400">{{ inventory.panel }} · {{ inventory.hostname }}</p>
                </div>
                <div class="flex shrink-0 items-center gap-3 text-xs font-medium text-slate-500 dark:text-slate-400">
                    <span class="flex items-center gap-1"><i class="bi bi-folder2"></i>{{ inventory.directories?.length || 0 }} directories</span>
                    <span class="flex items-center gap-1"><i class="bi bi-database"></i>{{ inventory.databases?.length || 0 }} databases</span>
                </div>
            </div>
            ...
            <div class="space-y-5 p-5">
                ...
                <div class="flex items-start gap-3 rounded-lg border border-indigo-200 bg-indigo-50 p-3.5 text-sm text-indigo-800 dark:border-indigo-800 dark:bg-indigo-500/10 dark:text-indigo-300">
                    <i class="bi bi-info-circle mt-0.5 shrink-0"></i><span>Only this file location and this database will be handled. Other websites and databases will not be included.</span>
                </div>

                <div class="flex flex-wrap gap-3">
                    <button type="button" :disabled="!selection.source_path || !!downloading" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:opacity-40" @click="download('files')">
                        <i class="bi bi-file-earmark-zip"></i>{{ downloading === 'files' ? 'Preparing files…' : 'Download selected files' }}
                    </button>
                    <button type="button" :disabled="!selection.source_database || !!downloading" class="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-40" @click="download('database')">
                        <i class="bi bi-database-down"></i>{{ downloading === 'database' ? 'Preparing database…' : 'Download selected database' }}
                    </button>
                </div>

                <div class="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700">
                    <div class="divide-y divide-slate-100 dark:divide-slate-800">
                        <div v-for="step in workflowSteps" :key="step.key" class="flex items-center justify-between gap-3 px-4 py-3">
                            <span class="flex items-center gap-2.5 text-sm text-slate-700 dark:text-slate-300">
                                <i :class="['bi', step.status === 'completed' ? 'bi-check-circle-fill text-emerald-500' : step.status === 'running' ? 'bi-arrow-repeat animate-spin text-indigo-500' : step.status === 'failed' ? 'bi-x-circle-fill text-red-500' : 'bi-circle text-slate-300 dark:text-slate-600']"></i>
                                {{ step.label }}
                            </span>
                            <span class="rounded-full px-2 py-0.5 text-xs font-semibold" :class="step.status === 'completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : step.status === 'running' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300' : step.status === 'failed' ? 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400'">{{ step.status }}</span>
                        </div>
                    </div>
                </div>

                <button type="button" :disabled="migrating || !targetDomain || !selection.source_path" class="inline-flex items-center justify-center gap-2 rounded-lg bg-violet-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-violet-700 disabled:opacity-40" @click="migrate">
                    <i class="bi bi-play-circle"></i>{{ migrating ? 'Running current step…' : 'Create & migrate step by step' }}
                </button>
            </div>
        </section>
    </div>
