...
class BackupSettings
{
    ...
    public function read(): array
    {
        ...
        try {
            ...
            return [
                ...
                'storage_driver' => (string) ($decoded['storage_driver'] ?? $defaults['storage_driver']),
                'account_delay_seconds' => max(0, (int) ($decoded['account_delay_seconds'] ?? $defaults['account_delay_seconds'])),
                'storage_configs' => is_array($decoded['storage_configs'] ?? null) ? $decoded['storage_configs'] : [],
    ...
    public function write(array $state): void
    {
        ...
        $normalized = [
            ...
            'storage_driver' => (string) ($state['storage_driver'] ?? 'local'),
            'account_delay_seconds' => max(0, min(3600, (int) ($state['account_delay_seconds'] ?? 5))),
            'storage_configs' => is_array($state['storage_configs'] ?? null) ? $state['storage_configs'] : [],
    ...
    private function defaults(): array
    {
        return [
            ...
            'storage_driver' => (string) env('BACKUP_STORAGE_DRIVER', 'local'),
            'account_delay_seconds' => max(0, (int) env('BACKUP_ACCOUNT_DELAY_SECONDS', 5)),
            'storage_configs' => [],
