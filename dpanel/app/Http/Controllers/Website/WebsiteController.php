...
CONF;
    ...
    protected function buildNginxVhostConfig(string $domain, string $rootPath, string $phpVersion): string
    {
        ...
        $hasTls = is_file($certificate) && is_file($privateKey);

        // With a certificate in place the plain-HTTP block only keeps the ACME challenge
        // reachable and redirects everything else, so an https page can never pull in
        // http assets generated by the backend.
        $listen = $hasTls
