...
class RestoreGenericWebsiteJob implements ShouldQueue
{
    ...
    public int $tries = 1;
    public function __construct(public string $importId) { $this->onQueue('heavy'); }
    ...
    public function handle(GenericWebsiteMigrationProvider $provider): void
    {
        ...
        try {
            ...
            DB::transaction(function () use ($settings): void {
                if (! empty($settings['sql_path'])) DatabaseRequest::updateOrCreate(['database_name' => $settings['database_name']], ['id' => (string) Str::uuid(),
        ...
        } catch (Throwable $e) { $import->update(['status' => 'failed', 'last_error' => $e->getMessage()]); throw $e; }
    }
}
